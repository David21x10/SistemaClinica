...
const RecetaReport = ({ diagnostico }: { diagnostico: Diagnostico }) => {
  const edad = diagnostico.paciente?.fecha_nacimiento 
    ? calculateAge(diagnostico.paciente.fecha_nacimiento) 
    : null;
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <View style={styles.leftHeader}>
            <Image style={styles.logo} src="/images/LogoRenueva.png" />
            <View style={styles.headerText}>
              <Text style={styles.institution}>CENTRO DE REHABILITACIÓN GABRIELA ALVARADO</Text>
              <Text style={styles.doctorInfo}>
                Terapeuta: {diagnostico.terapeuta ? `${diagnostico.terapeuta.nombre} ${diagnostico.terapeuta.apellido}` : 'N/A'}
              </Text>
            </View>
          </View>
        ...
        <View style={styles.patientSection}>
          <Text><Text style={styles.bold}>Paciente:</Text> {diagnostico.paciente ? `${diagnostico.paciente.nombre} ${diagnostico.paciente.apellido}` : 'N/A'}</Text>
