...
import BodegaReport from './Reports/BodegasReport';
...
function BodegaTable() {
    ...
    const obtenerBodegas = useCallback(() => {
        setLoading(true);
        axios.get('http://localhost:3002/Api/bodega/getbodegas')
            .then(response => {
                setBodegas(response.data.result || response.data);
            })
            .catch(error => {
    ...
    const eliminarBodegaHandler = (id: number) => {
        ...
        axios.delete(`http://localhost:3002/Api/bodegas/deletebodega?bodega_id=${id}`)
            .then(() => {
            ...
            .catch((err) => {
                console.error(err);
                toast.error(`Hubo un problema al eliminar: ${err.message || 'Error desconocido'}`);
            });
    };
    ...
    const bodegasFiltradas = bodegas.filter(b =>
        ...
        b.ubicacion.toLowerCase().includes(search.toLowerCase())
    );
    ...
    return (
        <Container fluid className="px-3 px-sm-4 px-md-5 py-4">
            ...
            <Card className="shadow-lg border-0" style={{ borderRadius: "20px", backgroundColor: "#ffffff" }}>
                <Card.Header className="bg-gradient py-3" style={{ backgroundColor: "#4a90e2", borderRadius: "20px 20px 0 0", border: "none" }}>
                ...
                <Card.Body className="p-3 p-md-4">
                    ...
                    {loading ? (
                        <div className="text-center py-5">
                            ...
                            <p className="mt-3 text-muted">Cargando registros de bodega...</p>
                        </div>
                    ) : bodegasFiltradas.length === 0 ? (
                        <div className="text-center py-5">
                            <p className="text-muted">No se encontraron registros que coincidan.</p>
                        </div>
                    ) : (
